...
void FourtyTwo() {
	...
	vi a(n + 10);
	rep(i, 0, n) cin >> a[i];
	unordered_map<int, array<int, 2>> mpp;
	mpp.reserve(1 << 19);
	rep(i, 0, n - 1) {
		rep(j, i + 1, n) {
			if (mpp.find(x - a[i] - a[j]) != mpp.end()) {
				array<int, 2> tmp = mpp[x - a[i] - a[j]];
				//cout << a[i] << " " << a[j] << " " << a[tmp[0]] << " " << a[tmp[1]] << nl;
				cout << i + 1 << " " << j + 1 << " " << tmp[0] + 1 << " " << tmp[1] + 1 << nl;
				return;
			}
		}
		for (int j = 0; j < i; j++) {
